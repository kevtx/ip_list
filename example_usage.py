...
def example_list_initialization():
    ...
    ip_list = IPList(ip_addresses=ip_addresses)
...
def example_mixed_usage():
    ...
    list_based = IPList(ip_addresses=list_ips)
...
def example_invalid_ips():
    ...
    try:
        IPList(ip_addresses=["192.168.1.1", "not-an-ip", "10.0.0.1"])
    except ValueError as e:
        print(f"Error with ignore_invalid=False: {e}")
    
    # With ignore_invalid=True, invalid IPs are skipped
    ip_list = IPList(
        ip_addresses=["192.168.1.1", "not-an-ip", "10.0.0.1"],
        ignore_invalid=True
    )
    print(f"With ignore_invalid=True: {ip_list}")
    print(f"Only valid IPs loaded: {sorted(ip_list.ip_list)}")
    print()
